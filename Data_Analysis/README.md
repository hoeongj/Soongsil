...
### 단계별 역량 (4개 phase로 본 진행)
1. **데이터 핸들링 (lab03–10):** `Table`로 선택·필터·정렬·**그룹/피벗**(`group`/`pivot`)·조인, `apply`로 열 단위 함수 적용, Matplotlib 시각화.
2. **시뮬레이션·확률 (lab11–12):** `np.random.choice`/`sample`로 몬테카를로 시뮬레이션, 확률·기댓값 추정.
3. **추론 통계 (lab13–22):** **순열검정**(permutation test), **TVD** 검정통계량, **A/B 테스트**, **부트스트랩 신뢰구간**, 정규분포·**중심극한정리**로 표본평균 변동성 분석.
4. **예측 모델링 (lab23–25):** 상관·산점도 → **최소제곱 선형 회귀**로 예측(FIFA 데이터).
